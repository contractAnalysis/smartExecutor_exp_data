...
result_folder=batin_group_${group_index}_results
rm -rf ${result_folder}
mkdir ${result_folder}
...
while IFS="," read solidity_name solc_version contract_name
  do
	...
	start=$(date +%s.%N)
	map analyze ${con_work_dir}${contract_folder}/$solidity_name:${contract_name}  | tee -a ${con_work_dir}${result_folder}/${solidity_name}__${contract_name}.txt
	end=$(date +%s.%N)       
