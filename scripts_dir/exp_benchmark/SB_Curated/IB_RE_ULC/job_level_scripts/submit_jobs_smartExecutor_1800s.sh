...
base_path='/scratch/06227/'+Constants.user+'/exp_benchmark/SB_Curated/IB_RE_ULC/smartExecutor_data/' # the path where all data required for the experiments are placed
...
# make sure each job have a different name, so that I can distinguish differnt jobs by names
 sbatch  -p  normal -n 1 -N 1 -t 2:10:00 -J job_$((i+1))_smartExecutor J26Container.sh  $((i*30))  ${group_name_prefix} ${run_script} ${job_1_container_script} ${base_path} ${con_work_dir} ${sif_name}
