...
while IFS="," read solidity_name solc_version contract_name
 ...
 echo "++++ ${solidity_name}  :  ${solc_version}  :  ${contract_name} ++++"  | tee -a ${con_work_dir}${result_folder}/${solidity_name}__${contract_name}.txt
	...
	start=$(date +%s.%N)
	 timeout 910 myth analyze ${con_work_dir}${contract_folder}/$solidity_name:${contract_name}  --create-timeout 60 --execution-timeout 900 -fdg -p2 0 | tee -a ${con_work_dir}${result_folder}/${solidity_name}__${contract_name}.txt
	...
	echo ${solidity_name}:${solc_version}:${contract_name}:${runtime1}:910:60:900 | tee -a ${con_work_dir}${result_folder}/${solidity_name}__${contract_name}.txt  
