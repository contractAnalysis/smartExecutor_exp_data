...
run_script='exp_run_smartExecutor_phase1_1800s.sh' # the script running within the container
...
# make sure each job have a different name, so that I can distinguish differnt jobs by names
 sbatch  -p  normal -n 1 -N 1 -t 6:00:00 -J job_$((i+1))_smartExecutor J30Container.sh  $((i*30))  ${group_name_prefix} ${run_script} ${job_1_container_script} ${base_path} ${con_work_dir} ${sif_name} &

done

i=13
 sbatch  -p  normal -n 1 -N 1 -t 6:00:00 -J job_$((i+1))_smartExecutor J27Container.sh  $((i*30))  ${group_name_prefix} ${run_script} ${job_1_container_script} ${base_path} ${con_work_dir} ${sif_name} &
