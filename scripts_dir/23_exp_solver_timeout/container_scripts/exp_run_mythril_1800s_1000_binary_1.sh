...
while IFS="," read solidity_name solc_version contract_name
  do
	...
	start=$(date +%s.%N)
	timeout 1900 myth analyze --codefile ${con_work_dir}${contract_folder}/${solidity_name}${contract_name}.bin  --create-timeout 60 --execution-timeout 1800 --solver-timeout 1000000 | tee -a ${con_work_dir}${result_folder}/${solidity_name}__${contract_name}_${tool_name}.txt
	end=$(date +%s.%N)       
