...
while IFS="," read solidity_name solc_version contract_name
  do
	...
	start=$(date +%s.%N)
	timeout 1810 myth analyze ${con_work_dir}${contract_folder}/$solidity_name:${contract_name}  --create-timeout 60 --execution-timeout 1800 -p 1 | tee -a ${con_work_dir}${result_folder}/${solidity_name}__${contract_name}.txt
	end=$(date +%s.%N)       
