...
result_folder=smartExecutor_binary_group_${group_index}_results_baseline10_1
rm -rf ${result_folder}
mkdir ${result_folder}
...
while IFS="," read solidity_name solc_version contract_name
 ...
 echo "++++ ${solidity_name}  :  ${solc_version}  :  ${contract_name} ++++"  | tee -a ${con_work_dir}${result_folder}/${solidity_name}__${contract_name}_${tool_name}.txt
	...
	start=$(date +%s.%N)
	 timeout 3780 myth analyze --codefile ${con_work_dir}${contract_folder}/${solidity_name}${contract_name}.bin  --create-timeout 60 --execution-timeout 3600 -fdg --random-baseline 10 --solver-timeout 1000000  | tee -a ${con_work_dir}${result_folder}/${solidity_name}__${contract_name}_${tool_name}.txt
