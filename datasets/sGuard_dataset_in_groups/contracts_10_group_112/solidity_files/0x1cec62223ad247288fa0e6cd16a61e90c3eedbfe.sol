...
contract OpenCourt is Context { 
    ...
    function payOpenCourt(string memory details) payable public { 
        openCourt.transfer(msg.value);
        emit OpenCourtPaid(_msgSender(), msg.value, details);
    }

    function updateJudge(address _judge) public onlyOpenCourt { 
        judge = _judge;
    }
    ...
    function updateJudgment(address _judgment) public onlyOpenCourt { 
        judgment = _judgment;
    }
