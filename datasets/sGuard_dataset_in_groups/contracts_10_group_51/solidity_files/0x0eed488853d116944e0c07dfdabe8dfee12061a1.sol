...
contract $USDAI is ERC20, ERC20Detailed {
    ...
    constructor () public ERC20Detailed("USDAI Dollar", "USDAI", 18) {} 
    ...
    function mint$USDAI(uint256 amount) public {
        ...
        $USDC.transferFrom(msg.sender, address(this), usdcSplit); 
        $DAI.transferFrom(msg.sender, address(this), daiSplit); 
        _mint(msg.sender, amount.mul($USDAIunit)); 
    }
    
    function redeem$USDAI(uint256 amount) public {
        uint256 usdcSplit = amount.mul($USDCunit).div(2);
        uint256 daiSplit = amount.mul($DAIunit).div(2);
        $USDC.transfer(msg.sender, usdcSplit); 
        $DAI.transfer(msg.sender, daiSplit); 
        _burn(msg.sender, amount.mul($USDAIunit)); 
    }
}