...
contract DMEX_Trading {
    ...
    uint256 public fundingInterval = 2057;     
    ...
    ) onlyAdmin returns (uint filledTakerTokenAmount)
    {
        ...
        {       
            ...
            {
                tv.makerBoolValues = [true, true, false]; 
            } else {               
                
                if (positionExists(t.makerPositionHash))
                {
                    
                    tv.makerBoolValues = [false, true, true]; 
                }
                else
                {
                    ...
                    tv.makerBoolValues = [false, true, false]; 
                }                
            }            
            ...
            {        
                ...
                tv.takerBoolValues = [true, false, false]; 
            } else {
                if (positionExists(t.takerPositionHash))
                {
                    
                    tv.takerBoolValues = [false, false, true]; 
                }
                else
                {    
                    ...
                    tv.takerBoolValues = [false, false, false]; 
                }
            }
        }        
        else 
        {      
            ...
            if (!positionExists(t.makerInversePositionHash) && !positionExists(t.makerPositionHash))
            {
                ...
                tv.makerBoolValues = [true, false, false]; 
                ...
                if (positionExists(t.makerPositionHash))
                {
                    ...
                    tv.makerBoolValues = [false, false, true]; 
                }
                else
                {
                    ...
                    tv.makerBoolValues = [false, false, false]; 
                }
            }    
            ...
            if (!positionExists(t.takerInversePositionHash) && !positionExists(t.takerPositionHash))
            {
                tv.takerBoolValues = [true, true, false]; 
            } else {
                if (positionExists(t.takerPositionHash))
                {
                    
                    tv.takerBoolValues = [false, true, true]; 
                }
                else
                {
                    ...
                    tv.takerBoolValues = [false, true, false]; 
                }
            }                      
        }
        ...
        if (tv.makerLoss > 0)
        {
            if (!updateBalances(
                    ...
                    !tv.makerBoolValues[0] && !tv.makerBoolValues[2] ? t.makerInversePositionHash : t.makerPositionHash, 
                    [
                        tv.qty, 
                        t.makerPrice, 
                        tv.makerFee, 
            ...
            updateBalances(
                ...
                !tv.takerBoolValues[0] && !tv.takerBoolValues[2] ? t.takerInversePositionHash : t.takerPositionHash,  
                [
                    tv.qty, 
                    t.makerPrice, 
                    tv.takerFee, 
    ...
    function calculateCollateral (uint256 qty, uint256 price, uint256 leverage, bytes32 futuresContractHash, uint256 fee) view returns (uint256) 
    {
        uint256 multiplier = futuresContracts[futuresContractHash].multiplier;
        uint256 collateral;
    ...
    function updateBalances (bytes32 futuresContract, address[3] addressValues, bytes32 positionHash, uint256[9] uintValues, bool[3] boolValues) private returns (bool)
    {
        ...
        if (pam[0] > 0)
        {
            addBalance(addressValues[0], feeAccount, DMEX_Base(exchangeContract).balanceOf(addressValues[0], feeAccount), pam[0]); 
        }
    ...
    function closeFuturesPositionInternal (bytes32 futuresContract, bool side, address user, address poolAddress, uint256 expirationFee) private returns (bool)
    {
        ...
        if (futuresContracts[futuresContract].broken) revert(); 
        if (futuresContracts[futuresContract].closed == false && futuresContracts[futuresContract].expirationBlock != 0) revert(); 
        if (pos[1] == 0) revert(); 
        if (pos[0] == 0) revert(); 
        ...
        addBalance(v.baseToken, feeAccount, DMEX_Base(exchangeContract).balanceOf(v.baseToken, feeAccount), safeMul(v.closingFee/2, 1e10)); 
    ...
    function updateMargin(bytes32 futuresContractHash, address user, bool side, uint8 vs, bytes32 r, bytes32 s, uint64 newMargin , uint256 operationFee )
    {
    ...
    function liquidatePosition (bytes32 positionHash, bytes32 futuresContractHash, address user, bool side, uint256 price, address poolAddress, uint256 block) private
    {
        ...
        if (pos[0] == 0) return;
        if (!pools[poolAddress]) return;  
        if (futuresContracts[futuresContractHash].broken) revert(); 
