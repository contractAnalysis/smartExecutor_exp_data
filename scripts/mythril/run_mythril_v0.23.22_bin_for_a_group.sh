...
result_folder=${tool_name}_bin_results_${tool_timeout}s_${run_idx}
...
while IFS="," read solidity_name solc_version contract_name 
 ...
 echo "++++ ${solidity_name}  :  ${solc_version}  :  ${contract_name} ++++"  | tee -a ${result_dir}${solidity_name}__${contract_name}_${tool_name}_${tool_timeout}_${run_idx}.txt
	...
	start=$(date +%s.%N)
	timeout ${cli_timeout} myth analyze --codefile ${contract_dir}${bin_folder}/${solidity_name}__${contract_name}/${contract_name}.bin  --create-timeout 60 --execution-timeout ${tool_timeout} --solver-timeout ${solver_timeout} --pruning-factor 1.0 | tee -a ${result_dir}${solidity_name}__${contract_name}_${tool_name}_${tool_timeout}_${run_idx}.txt
